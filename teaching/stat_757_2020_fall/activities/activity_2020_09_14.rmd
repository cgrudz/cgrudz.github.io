...
Suppose that the linear model satisfies, $Y_i = \beta_0 + \epsilon_i$,
but that the other assumptions of the standard model hold. 
Recall the definitions,
$$\begin{align}
RSS = \sum_{i=1}^n \left(Y_i - \hat{Y}_i\right)^2 & &TSS = \sum_{i=1}^n \left(Y_i - \overline{Y}\right)^2
\end{align}$$
Derive the values of $\mathbb{E}\left[RSS\right]$ and $\mathbb{E}\left[TSS\right]$.  What is the implication for $R^2$?
...
such that the $TSS$ is a random variable, defined as a combination of the random variables $Y, \overline{Y}$. What is the value of the expectation of the $TSS$?  Consider using the definition of the regression equation as a substitution for $\overline{Y}$ and $Y_i$.
