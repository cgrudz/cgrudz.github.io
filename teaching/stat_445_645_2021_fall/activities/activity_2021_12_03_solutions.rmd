...
$$\begin{align}
L_\mathbf{x}(\lambda) & = \log(\mathcal{L}_{\mathbf{x}}(\lambda)) \\
&= \sum_{i=1}^n \log( f_\xi(x_i,\lambda)) \\
...
neg_pois_log_like <- function(lambda) {
  ...
  like <- sum(-log(lambda) * x + lambda + log(factorial(x)))
  return(like)
}
neg_pois_log_like(10)
...
lik <- 1:100
for (i in 1:100){
  lik[i] <- neg_pois_log_like(i)
}
plot(lik, xlab="Lambda", ylab="Negative log-likelihood")
```


We see in the above plot that the negative log-likelihood function is convex with a minimum at the true value.
