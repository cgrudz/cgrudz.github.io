...
We define the Gaussian likelihood function as follows:
```{r}
gauss_like <- function(x, params){
  mu <- params[1]
  sigma <- params[2]
  n <- length(x)
  delta <- x - mu
  like <- -0.5 * sum((delta / sigma)^2)
  like <- (1 / (sigma * sqrt(pi *2)))^n * exp(like)
  return(like)
}
```


Then we define the initial sample
```{r}
set.seed(0)
x <- rnorm(20)
x
```
and loop through the parameter values.
```{r}
for (i in 1:ncol(param_vals)){
 print(gauss_like(param_vals[,i], x))
}
```
Note, we indeed find the largest likelihood at the true parameters here but it may not be the case over all values.

Now, we define the second sample,
```{r}
set.seed(0)
x <- rnorm(1000, 0, 1)
```
and compute the likelihood of the parameters
```{r}
for (i in 1:ncol(param_vals)){
 print(gauss_like(param_vals[,i], x))
}
```
to find that all have converged to zero.  This is because for large $n\gg 1$, this will rapidly converge to numerical zero due to the exponential decay along with the power decay in the product.
...
#### Solution:
If we take the log of the above likelihood function, we will obtain,
...
neg_gauss_log_like <- function(params){
  # the observations can be defined in the function body here for simplicity
  set.seed(0)
  n <- 20
  x_sample <- rnorm(n)
...
fBFGS = optimx(fn = neg_gauss_log_like, # define the objective function
   ...
   x = x_sample # supply the data as a keyword argument
)
print(fBFGS)
```
...
neg_gauss_log_like <- function(params){
  ...
  delta <- x - mu
  like <- 0.5 * sum((delta / sigma)^2)
  like <- like +  n*log(sigma * sqrt(2*pi))
  return(like)
}
