...
<h1 style='color:black'> Simple linear regression -- part 2</h1>
<h2 style='color:black'> 09/05/2019</h2>
...
<ul> 
  <li>Supposing that the relationship in simple linear regression is valid, we can create an objective function which measures:</li>
    <ol>
      ...
      <li>with the arguments $\overline{\beta}_0,\overline{\beta}_1$ as unkown variables.</li>
    </ol>
...
<ul>
  ...
  $$\begin{align}
  \hat{\beta}_1 &= \frac{\sum_{i=1}^n\left(X_i - \overline{X}_i \right)\left(Y_i - \overline{Y}_i\right)}{\sum_{i=1}^n\left(X_i - \overline{X}_i \right)}\\
...
<h3>The "normal" equations -- continued</h3>

<ul>
  <li><b>Exercise (5 minutes):</b> use the definition of the objective function
  $$J\left(\overline{\beta}_0,\overline{\beta}_1\right) = \sum_{i=1}^n \left(Y_i - \overline{\beta}_0 - \overline{\beta}_1 X_i\right)^2,$$</li>
  <li>and the definition of $\hat{\beta}_0,\hat{\beta}_1$ as the minimizers of $J$,</li>
  <li> to derive the normal equations for $\hat{\beta}_0,\hat{\beta}_1$,
  $$\begin{align}
  \sum_{i=1}^n Y_i &= n\hat{\beta}_0 + \hat{\beta}_1 \sum_{i=1}^n X_i \\
  \sum_{i=1}^n X_i Y_i & =  \sum_{i=1}^n \left(\hat{\beta}_0 X_i +  \hat{\beta}_1 X_i^2\right)
  \end{align}$$</li>
...
<ul>
  <li><b>Solution:</b> the objective function is convex with a global minimum, and therefore has a single critical point corresponding to this minimum.</li>
  <li> In each of the arguments, $\overline{\beta}_0$ and $\overline{\beta}_1$, we can take the partial derivative of the objective function,
  $$\begin{align}
  ...
  \end{align}$$</li>
  <li>Setting these partial derivatives to zero when evaluated at $\hat{\beta}_0,\hat{\beta}_1$, we find
    $$\begin{align}
  ...
  \end{align}$$</li>
  <li>By re-arranging terms, we find the normal equations,
  $$\begin{align}
  \sum_{i=1}^n Y_i &= n\hat{\beta}_0 + \hat{\beta}_1 \sum_{i=1}^n X_i \\
  \sum_{i=1}^n X_i Y_i & =  \sum_{i=1}^n \left(\hat{\beta}_0 X_i +  \hat{\beta}_1 X_i^2\right)
  \end{align}$$</li>
...
<h3>Properties of the estimated regression function -- exercises</h3>

<ul>
  <li><b>Exercises:</b> with a partner, use the definition of the normal equations
  $$\begin{align}
  \sum_{i=1}^n Y_i &= n\hat{\beta}_0 + \hat{\beta}_1 \sum_{i=1}^n X_i ,\\
  \sum_{i=1}^n X_i Y_i & =  \sum_{i=1}^n \left(\hat{\beta}_0 X_i +  \hat{\beta}_1 X_i^2\right),
  \end{align}$$</li>
  <li>and the definition of the residuals,
  $$\begin{align} \hat{\epsilon}_i &= Y_i - \hat{Y}_i ,\end{align}$$</li>
  <li>to prove the following statements:</li> 
  <ol>
    <li>The sum of the residuals is zero, i.e.,
    $$\sum_{i=1}^n \hat{\epsilon}_i = 0$$</li>
    <li>The sum of the observed values equals the sum of the fitted values,
...
<h3>Properties of the estimated regression function -- solutions</h3>

<ul>
  <li><b>Solution:</b> Notice that the first of the normal equations gives us
  $$\begin{align}
  &\sum_{i=1}^n Y_i = n\hat{\beta}_0 + \hat{\beta}_1 \sum_{i=1}^n X_i\\
  ...
  <li>Furthermore, by definition of the residuals, we have
  $$\begin{align}
  ...
  \end{align}$$
so that the last statement proves the second equation.</li>

<li>The remaining statements can be proven similarly --- this will come up in our homework and quizzes.</li>
...
<ul>
  ...
  $$Y = \hat{\beta}_0 + \hat{\beta}_1 X.$$</li>
  ...
  $$\hat{Y}_i = \hat{\beta}_0 + \hat{\beta}_1;$$</li>
...
<ul>
  ...
  <li>However, this sum of square deviations needs a normalization term;</li>
    ...
    $$s^2 \triangleq \frac{1}{n-1}\sum_{i=1}^n \left(Y_i - \overline{Y}_i\right)^2.$$
    </li>
</ul>
...
<ul>
  ...
  $$\begin{align}RSS &= \sum_{i=1}^n \left(Y_i - \overline{Y}_i\right)^2\\
